...
#record
record_path = './drone_playing_record_3000.tw'

#model
date_time = datetime.datetime.now().strftime("%Y%m%d-%H-%M-%S")
model_path = './drone_agent_awr_advanced_' + date_time + '.pt'
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if done:
            ...
            if episode % save_interval == 0 and episode > start_interval:
                agent.save(model_path)

            if episode % test_interval == 0 and episode > start_interval:
                env_info = env.reset(train_mode=train_mode)[default_brain]
                state_test = env_info.vector_observations[0]
                done_test = False
                running_reward_test = []

                while not done_test:
                    action_test = agent.get_action(state_test)
                    env_info = env.step(action_test)[default_brain]
                    state_test = env_info.vector_observations[0]
                    reward_test = env_info.rewards[0]
                    done_test = env_info.local_done[0]

                    running_reward_test.append(reward_test)

                success_test = reward_test == 1
                print('episode: {}, reward : {}, success : {}'.format(episode, np.sum(running_reward_test), success_test))

            if episode % train_interval == 0 and episode > start_interval:
